...
import {StyleSheet, Modal, Text, View, Button, Vibration, Dimensions, ScrollView, TouchableOpacity} from 'react-native'
...
class Timer extends React.Component{
    ...
    countdown = () =>{
        ...
        this.interval = setInterval(this.timeCheck, 1000)
    }
    ...
    timeCheck = async() =>{
        if( this.state.activeProfile !== ''){
            if(this.state.minute === 0 && this.state.second === 1){
                ...
                //try{
                    //await soundObject.loadAsync(require('./assets/blip.wav'));
            ...
            if(this.state.minute === 0 && this.state.second === 0 ){
                ...
                if( this.state.isBreak === true && this.state.title!=='Break'){
                    this.setState({hour: this.state.breakHr, minute: this.state.breakMin , second: this.state.breakSec, title:'Break'})
                }
                else if(this.state.repeat > 0){
                    let currentAct = this.state.activeProfile.userActivities[this.state.position]

                    this.setState({ hour: currentAct.hour,
                    ...
                    if(this.state.position !== this.state.index){
                        let nextActivity = this.state.activeProfile.userActivities[this.state.position]
                        this.setState({hour:  nextActivity.hour,
    ...
    render(){
        return<View style={(this.state.title === "Break"?styles.appBreak:styles.app)}>
            ...
            { !this.state.modalVisible && <AwesomeButton
                style={styles.openButton}
                stretch={true}
            ...
            <View style={styles.timer}>
                <Text style={styles.numbers}>{(this.state.hour<10?'0':'') + this.state.hour}</Text>
                <Text style={styles.numbers}>:</Text>
                <Text style={styles.numbers}>{(this.state.minute<10?'0':'') + this.state.minute}</Text>
                <Text style={styles.numbers}>:</Text>
                <Text style={styles.numbers}>{(this.state.second<10?'0':'') + this.state.second}</Text>
            </View>
            ...
            <View style={styles.buttonRow}>
                <View style={styles.buttons}>
                    <Icon.Button
                        ...
                        onPress={this.state.isRunning === true?this.pause:this.countdown}
