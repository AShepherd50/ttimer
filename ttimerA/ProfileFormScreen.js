...
import {View, StyleSheet, TextInput, ScrollView, Text, Button, Dimensions} from 'react-native'

const DisplayActivity = props =>{
    if(props.activity.isBreak) {
        return (
            <View style={styles.set}>
                    <View style={styles.activityId}><Text style={styles.textId}>{props.activity.key}</Text></View>
                    <View style={styles.activityContent}>
                        ...
                        <Text style={styles.text}>Time:
                            ...
                            {props.activity.minute === "" ? '00' : (props.activity.minute < 10 ? 0 : "") + props.activity.minute}:
                            {props.activity.sec === "" ? '00' : (props.activity.sec < 10 ? 0 : "") + props.activity.sec}
                        </Text>
                        <Text style={styles.text}>Cycles: {props.activity.repeat}</Text>
                        <Text style={styles.text}>Break per Cycle:
                            {props.activity.breakHr === "" ? '00' : props.activity.breakHr}:
                            {props.activity.breakMin === "" ? '00' : (props.activity.breakMin < 10 ? 0 : "") + props.activity.breakMin}:
                            {props.activity.breakSec === "" ? '00' : (props.activity.breakSec < 10 ? 0 : "") + props.activity.breakSec}</Text>
                    </View>
...
export default class ProfileFormScreen extends React.Component{
    ...
    handleSubmit=()=>{
        ...
        for(let i=0; i<index; i++){
            hr =  hr + (parseInt(this.state.userActivities[i].breakHr) + parseInt(this.state.userActivities[i].hour) * parseInt(this.state.userActivities[i].repeat));
            min = min + (parseInt(this.state.userActivities[i].breakMin) + parseInt(this.state.userActivities[i].minute) * parseInt(this.state.userActivities[i].repeat));
            second = second + (parseInt(this.state.userActivities[i].breakSec)+ parseInt(this.state.userActivities[i].sec) * parseInt(this.state.userActivities[i].repeat));
        }
    ...
    deleteActivity=props=>{
        this.setState({userActivities: this.state.userActivities.filter(activity => activity.key !== props)})
    }
        ...
        if(this.props !== prevProps) {
            ...
            this.setState(prevState=>({
                userActivities: [...this.state.userActivities, {
                    ...
                    breakHr: (this.props.route.params.breakHr === undefined?0:this.props.route.params.breakHr),
                    breakMin: (this.props.route.params.breakMin === undefined?0:this.props.route.params.breakMin),
                    breakSec: (this.props.route.params.breakSec === undefined?0:this.props.route.params.breakSec)
                }],
            }))
        }
    }
        ...
        return(
            <View>
                <View style={styles.top}>
                    <Text style={{paddingRight: 10, paddingTop: 5, fontWeight: 'bold', fontSize: 16}}>Profile Name:</Text>
                    <TextInput
                        style={styles.input}
                ...
                <ScrollView>
                    {this.state.userActivities.map(activities=>(
                        <DisplayActivity
                            key={activities.key}
                            activity={activities}
                            onDelete={()=>{this.deleteActivity(activities.key)}}
                        />
                    ))}
                </ScrollView>
