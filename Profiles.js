    ...
    <View style={styles.profile}>
      ...
      <View style={styles.profileContent}>
        ...
        <Text style={styles.text}>
          Number of Activities: {props.profile.userActivities.length}
        </Text>
        <Text style={styles.text}>
          Total Time: {props.profile.totalHr}:
          {props.profile.totalMin < 10
            ? '0' + props.profile.totalMin
            : props.profile.totalMin}
          :
          {props.profile.totalSec < 10
            ? '0' + props.profile.totalSec
            : props.profile.totalSec}
        </Text>
      </View>
...
export default class Profiles extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    let x = this.state.keys.sort((a, b) => a - b);
    let index = x.pop();
    ...
    if (this.props !== prevProps) {
      if (this.props.route.params.profile.key === '') {
        ...
        this.setState(prevState => ({
          ...
          keys: [...this.state.keys, this.state.nextKey],
  ...
  handleDelete = props => {
    this.setState({
      profiles: this.state.profiles.filter(profile => profile.key !== props),
      nextKey: parseInt(this.state.nextKey) + 1,
    });
    removeData(props);
  };
